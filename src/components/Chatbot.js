...
const Chatbot = ({ isOpen, onClose, properties: propsProperties }) => {
  ...
  // Load fresh property data when chatbot opens
  useEffect(() => {
    if (isOpen) {
  ...
  // Use props properties as fallback
  useEffect(() => {
  ...
  const loadLatestProperties = async () => {
    try {
      ...
      console.error('Error loading properties for chatbot:', error);
      // Fallback to props properties if database fails
      if (propsProperties) {
        setProperties(propsProperties);
      }
    }
